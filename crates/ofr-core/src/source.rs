//! 復旧元の解決と、安全原則(PLAN.md 6章)のチェック。
//!
//! デバイス ID かイメージファイルを開き、その中のどこにファイルシステムが
//! あるかを割り出す。`ofr image` で取ったイメージはデバイス丸ごとなので、
//! パーティションテーブルを読んでボリュームの開始位置を探す必要がある。
//!
//! ここは CLI と GUI の両方が通る道にしてある。**復旧元への書き込み禁止**と
//! **起動ディスク / 同一デバイスの拒否**は復旧ソフトの根幹なので、
//! 判定を 2 か所に分けて片方だけ直す事故を起こさないため。
...
pub fn open_source(source: &str) -> Result<Arc<dyn Device>> {
    let path = Path::new(source);
    if path.is_file() {
        Ok(Arc::new(FileDevice::open(path)?))
    } else {
        Ok(Arc::from(ofr_device::open_device(source)?))
    }
}

/// 復旧元として選んでよいデバイスか、列挙情報だけで確かめる(PLAN.md 6章 3項)。
///
/// デバイスを開く前に呼ぶ。開くには管理者権限が要るので、権限エラーで
/// 「起動ディスクだから駄目」という本当の理由が隠れないようにする。
pub fn check_source_selectable(source: &str) -> Result<()> {
...
pub fn check_destination(source_id: &str, dest: &Path) -> Result<()> {
...
pub fn locate(device: &dyn Device, wanted: Option<FsKind>, offset: Option<u64>) -> Result<Volume> {
...
/// 指定された種別でボリュームを開く。
pub fn open_filesystem(region: &dyn Device, kind: FsKind) -> Result<Box<dyn FileSystem + '_>> {
    Ok(match kind {
...
mod tests {
    ...
    fn finds_a_bare_fat32_volume() {
        ...
        let volume = locate(&device, None, None).unwrap();
    ...
    fn honours_an_explicit_offset() {
        let device = mock(Fat32Image::new(48 << 20).build());
        let err = locate(&device, None, Some(1 << 20)).unwrap_err();
        assert_eq!(err.code(), crate::ErrorCode::NoFilesystem);
    }

    #[test]
    fn refuses_to_open_a_volume_as_the_wrong_type() {
        let device = mock(Fat32Image::new(48 << 20).build());
        assert!(locate(&device, Some(FsKind::ExFat), None).is_err());
    }
