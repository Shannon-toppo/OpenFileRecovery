//! 項目の絞り込み。CLI の `--include` / `--status` と GUI の検索欄で同じ挙動にする。
...
impl Filter {
    ...
    /// 状態コード(`deleted` など)から条件を組み立てる。知らないコードは無視する。
    pub fn with_status_codes(mut self, codes: &[String]) -> Self {
        self.statuses = codes.iter().filter_map(|c| status_from_code(c)).collect();
        self
    }
...
/// 状態コードから [`EntryStatus`] へ。
pub fn status_from_code(code: &str) -> Option<EntryStatus> {
    Some(match code {
        "intact" => EntryStatus::Intact,
        "deleted" => EntryStatus::Deleted,
        "orphaned" => EntryStatus::Orphaned,
        "damaged" => EntryStatus::Damaged,
        _ => return None,
    })
}
...
mod tests {
    ...
    fn filters_by_pattern_and_status() {
        let filter = Filter {
            ...
            statuses: vec![EntryStatus::Deleted],
        };
    ...
    fn reads_status_codes() {
        let filter = Filter::default().with_status_codes(&[
            "deleted".to_string(),
            "orphaned".to_string(),
            "なにこれ".to_string(),
        ]);
        assert_eq!(
            filter.statuses,
            vec![EntryStatus::Deleted, EntryStatus::Orphaned]
        );
    }
}
